...
reviews_xlsx = os.path.join(BASE_DIR, r"C:\Users\Sanjali\Desktop\tourism_pipeline\Final_Cleaned_Tourist_Reviews.xlsx")
activities_csv = os.path.join(BASE_DIR, r"C:\Users\Sanjali\Desktop\tourism_pipeline\Rural_Activities_Expanded.csv")
...
page_backgrounds = {
    ...
    "Itinerary": r"C:\Users\Sanjali\Downloads\sri-lankan-travel-map.png",
    "About": r"C:\Users\Sanjali\Downloads\511564047_3690789407732651_2711082666974816646_n.jpg"
}
...
# -------------------- Explore Page --------------------
    ...
    if not reviews.empty:
        ...
        # Pie chart: Review Distribution by Area
        ...
        # Urban vs Rural sentiment comparison
...
# -------------------- Itinerary Page --------------------
    ...
    # Two-column layout: Left = Form + Itinerary, Right = Map
        ...
        if not reviews_df.empty and not activities_df.empty:
            with st.form("itinerary_form"):
                ...
                preferred_district = st.selectbox(
                    "📍 Preferred District",
                    options=["Any"] + sorted(reviews_df['District'].dropna().unique())
                )
                preferred_activity = st.multiselect(
                    "🎯 Preferred Activity Category",
                    options=["Any"] + sorted(activities_df['Activity Category'].dropna().unique())
                )
            ...
            if submitted:
                ...
                # Merge activity info by District + Destination if exists
    ...
    with col2:
        st.image(r"C:/Users/Sanjali/Downloads/sri-lankan-travel-map.png", width='stretch')
...
# -------------------- About Page --------------------
    ...
    # Two-column layout: Left = About text, Right = Image
    col1, col2 = st.columns([2, 1])

    about_text = """
    <h1>🍃 About TravelPulse Sri Lanka</h1>
    <p><b>TravelPulse Sri Lanka</b> is more than just a travel guide – it is a data-driven platform designed to help travelers discover the island in a way that is meaningful, authentic, and sustainable. By applying advanced sentiment analysis to thousands of genuine tourist reviews, TravelPulse reveals what visitors truly value about Sri Lanka, from the vibrant pulse of its bustling cities to the tranquil beauty of its untouched rural landscapes. Every insight is drawn from real experiences, making TravelPulse a trusted companion for travelers who want more than just recommendations — they want journeys that matter.</p>
    <p>Our mission is to promote responsible tourism that not only creates unforgettable memories for travelers but also strengthens local communities, protects cultural traditions, and safeguards the island’s natural treasures for generations to come. To achieve this, TravelPulse goes beyond being a simple guidebook — it acts as a bridge between travelers and authentic experiences. By highlighting both world-renowned attractions and hidden gems, including rural destinations that are often overlooked, the platform helps distribute tourism more evenly across the island. This approach eases pressure on popular hotspots while also empowering smaller communities and supporting sustainable development, ensuring that travel benefits everyone.</p>
    ...
    <p>✨ With TravelPulse Sri Lanka, every trip becomes more than just a holiday — it is an opportunity to travel with purpose, live like a local, and create memories that leave a lasting impact on both you and the island itself.</p>
    """
    ...
    with col2:
        st.image(r"C:/Users/Sanjali/Downloads/Jaffna Sri Lanka Aesthetic.jpeg", width='stretch')
=======
import os
import sqlite3
from io import BytesIO
...
# -------------------- About Page --------------------
    ...
    with col2:
        st.image(r"C:/Users/Sanjali/Downloads/Jaffna Sri Lanka Aesthetic.jpeg", width='stretch')
>>>>>>> 4e75e4213a0bd3db7a6dbb32ef6ae428c03ebf67
